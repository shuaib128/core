...
class ProfileUpdateAPIView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if image_data is not None:
            # assuming that imageData is in format "data:image/png;base64,iVBORw0KG..."
            format, imgstr = image_data.split(';base64,')
            ext = format.split('/')[-1]
            profile.profile_picture.save(f'{profile.id}_profile_picture.{ext}', ContentFile(
                base64.b64decode(imgstr)), save=False)
            profile.save()
...
class SearchUserView(APIView):
    ...
    def post(self, request):
        if request.data != "":
            filtered_users = Profile.objects.filter(
                username__contains=request.data)
